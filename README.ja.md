...
エージェントへ指示を送ったあと別の作業へ移っても、返信完了は目立つ `完了！`、許可要求などは `確認待ち` と実際のセッション名で知らせます。Claude CodeとCodexの複数タスクを一つの画面で見渡し、タスクキーを押して元のアプリへ戻れます。
...
- Claude CodeとCodexの `作業中`、`完了！`、`確認待ち`、`エラー` を表示
- 両エージェントの実タスクを最終更新日時の新しい順に2ページで最大13件表示
- 変更可能なClaude/Codexのチャット名をセッション名として使用
- 返信完了後の10秒間はキー全面を高輝度の赤と黄色で交互に点滅し、その後は更新からの経過時間を表示しながら、対象のタスクキーまたは概要キーを実際に押すまで赤い完了表示を維持
...
| 完了！ | エージェントの返信が完了し、ユーザーの確認を待っている |
| 確認待ち | 回答・選択・実行許可が必要 |
| エラー | ツールまたは応答が失敗・中断 |
...
タスクキーを短押しすると、完了の強調表示を通常の `確認待ち` に戻し、そのセッションを開いている可能性が最も高いアプリを前面にします。タスクは一覧に残ります。`確認待ち` のキーを0.8秒以上長押しすると、その返信を完了扱いにしてタスク一覧から外します。作業中のタスクを長押ししても一覧から外しません。
...
ClaudeまたはCodexの概要キーを1回押すと、そのエージェントの完了強調表示を通常状態へ戻します。統合概要キーでは両エージェントが対象です。タスクは一覧に残り、通常の `確認待ち` を維持します。利用率キーは押すと再描画します。
